\*\*DEV\_DesignArchitecture\_10.2\_20260120 – Consolidated Architecture Specification\*\*    
\*\*Last Updated: 2026-01-20\*\*    
\*\*Status: Locked / Production-Ready\*\*    
\*\*Supersedes: All prior V10.x documents and addendums\*\*  

\---

\#\#\# \*\*0. Document Change Log & Supersession Notice\*\*

\*\*This V10.2 document is a complete consolidation of the frozen V10.0 architecture and all subsequent normative addendums through 10.1.10.\*\* All prior documents are hereby deprecated and replaced by this single source of truth.
...
\- DEV\_DesignArchitectureAddendum\_10.1.3\_20260104.txt (integrated into Section 6.4)  
...
\#\#\# \*\*0.1 Addendum Integration Log (2025-12-30 → 2026-01-18)\*\*
...
| 10.1.3 | 2026-01-04 | \*\*Addition\*\* | 6.4 Days Remaining Semantics |  
...
| Frontend | Next.js 14 \+ CSS Modules \[Updated per Addendum 10.3\] | Fast, responsive, AI-generable UI |  
...
\#\#\#\# \*\* 6.4 Days Remaining Semantics — Threshold-Driven Action Trigger \[New per Addendum 10.1.3\] \*\*

\*\* Authoritative Definition \*\*    
\`daysOfCashRemaining\` represents: The number of days until forecasted balance falls below the user-defined action threshold. It does NOT represent: days until bankruptcy, days until balance reaches zero, or an immutable danger condition.

\*\* Threshold Semantics \*\*    
\- \`warningThreshold\` : User-defined, intentionally affects daysOfCashRemaining, represents an action trigger—not a failure state—allowing users to model personal risk tolerance.  
\- \*\* Danger condition (balance \< 0): \*\* Still relevant for red/critical UI states and semantic cash exhaustion, but not required to drive daysOfCashRemaining.

\*\* Backend Behavior (Locked) \*\*    
\- Backend continues to accept threshold in forecast requests and compute daysOfCashRemaining using that threshold.  
\- No separation between "warning" and "action" thresholds is required at MVP stage.  
\- No regression exists.

\*\* Frontend Responsibility \*\*    
\- \*\* MUST \*\* clearly communicate that warning threshold affects "days remaining" and is intentional/user-controlled.  
\- \*\* MUST NOT \*\* describe daysOfCashRemaining as "days until zero" or imply backend inconsistency.
...
\#\#\#\# \*\* 18.2 Repository Separation Model \[New per Addendum 10.1.6\] \*\*
...
2\. \*\* fortunetell-app \*\* – Primary product runtime (app.fortunetell.app)  
...
\- \*\* fortunetell-backend: \*\* Backend scope is closed for Free \+ Essential MVP. Changes allowed only for proven correctness bugs or gated prerequisites.  
\- \*\* fortunetell-app: \*\* Must remain compliant with V10.2 lifecycle, storage, and UI doctrine.  
...
\#\#\#\# \*\*21.6 Icon System\*\*  
\- All SVG icons MUST derive width/height from icon size tokens (\`--icon-size-sm\`, \`--icon-size-md\`, \`--icon-size-lg\`).  
\- \`--icon-size-lg\` is canonical system icon size (Dock icons, history, back arrow, close X).  
\- \`--icon-size-sm/md\` MAY be used only for explicitly documented smaller roles.  
\- Icon colors MUST come from icon/text tokens (\`--icon-color-\*\`, \`--app-text-\*\`, \`--text-on-\*\`). New raw color values for icons are forbidden unless explicitly requested.
...
\#\#\#\# \*\*31.2 DEV\_DesignTokensSystem\_1.3\_20260115 OR current authoritative version (Normative Inclusion)\*\*  
The entire content of \`DEV\_DesignTokensSystem\_1.3\_20260115.txt\` is incorporated without modification.    
\*\* Source: \*\* \`/mnt/data/DEV\_DesignTokensSystem\_1.3\_20260115.txt\`    
...
\- Tokens: DEV\_DesignTokensSystem\_1.3 adopted even if marketing shell not yet live
...
\#\#\#\# \*\* 31.8 Enforcement \*\*  
\- Any new surface must use tokens from DEV\_DesignTokensSystem\_1.3 and respect domain separation  
\- Any proposal adding new styling technology, non-token colors/typography, or changing domain routing must be captured in future DEV\_DesignArchitectureAddendum\_10.1.x
...
This section does not change forecast logic, introduce new APIs, modify storage, add AI computation, alter tier functionality, or require immediate code changes. V10.2 remains single source of truth for system behavior.

\#\#\#\# \*\* 32.2 Determinism as External Contract \*\*  
\*\* Existing V10.2 Reality (Restated): \*\*  
...
\#\#\# \*\*Document Change Summary (V10.0 → V10.1 → V10.1.10 → V10.2)\*\*
...
1\. \*\*Import/Export Operational Clarity\*\* (10.1.1)  
   \- Explicit user-controlled operations  
   \- Auto-export best-effort convenience only  
   \- Non-recursive import/export relationship

2\. \*\*AdSense Integration & Privacy Posture\*\* (10.1.2)  
   \- Complete ad provider specification  
   \- Non-personalized targeting only  
   \- Comprehensive surface definitions and error handling  
   \- Minimal telemetry requirements

3\. \*\*Days Remaining Threshold Semantics\*\* (10.1.3)  
   \- Threshold-driven calculation formalized  
   \- Clarified frontend communication requirements

4\. \*\*Design Tokens System & Domain Strategy\*\* (10.1.4)  
   \- Unified token system across app \+ marketing  
   \- Domain separation (app.fortunetell.app vs fortunetell.app)  
   \- MDX content management strategy  
   \- Interim simple-budget.app routing

5\. \*\*AI Discovery & Semantic Integrity\*\* (10.1.5)  
   \- Determinism as external contract  
   \- Documentation constraints for AI-extractable descriptions  
   \- Domain ownership rules

6\. \*\*Repository Separation for AI-Risk Containment\*\* (10.1.6)  
   \- Four-repo model formalized  
   \- Unified token handling rules  
   \- CI guardrail expectations

7\. \*\*Token-Only, Component-Scoped Styling\*\* (10.1.7)  
   \- Complete replacement of CSS architecture  
   \- Strict token usage enforcement  
   \- AI execution rules

8\. \*\*Frontend Layout & Container System\*\* (10.1.8)  
   \- Canonical view names and routes (Main, Account, Settings)  
   \- Shell and global utility responsibilities  
   \- Flat vs card layout rules  
   \- Divider system

9\. \*\*Button Semantics & CSS Cleanup Protocol\*\* (10.1.9)  
   \- Primary/Secondary/Danger classification rules  
   \- Calendar day button and icon button exceptions  
   \- CSS cleanup constraints  
   \- Media query breakpoint rules

10\. \*\*Frontend Implementation Status Documentation\*\* (10.1.10)  
    \- Account/Settings pattern documentation  
    \- Legacy area acceptance  
    \- Explicitly deferred topics

\#\#\#\# \*\*V10.2: Version Alignment & Metadata Cleanup\*\*  
\- \*\*No behavioral changes beyond V10.1.10.\*\* Consolidates V10.1 \+ all 10.1.x addenda into a single spec and corrects version/metadata and token-file references from 1.0 to 1.3.
...
\*\*End of DEV\_DesignArchitecture\_10.2\_20260120 – Consolidated Architecture Specification\*\*